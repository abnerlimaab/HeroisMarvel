...
class HeroesTableViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        vc.hero = heroes[tableView.indexPathForSelectedRow?.row ?? 0]
    }
    
    func loadHeroes() {
        loadingHeroes = true
        MarvelAPI.loadHeros(mame: name, page: currentPage) { info in
            if let info = info {
                ...
                DispatchQueue.main.async {
                    ...
                    self.label.text = "Não foi encontrado heróis com o nome \(self.name ?? "")"
